...
iter_per_epoch = 30 #Set accordingly depending on the resource and needs
...
class Network(object):
    ...
    def run(self):
        ...
        scheduler_1 = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer_1, mode="max", min_lr = 0.000001, patience=50, verbose=True)
        ...
        for epoch in range(1, epochs):
            ...
            semi_dataloader = iter(zip(cycle(train_loader), cycle(unlabeled_loader))) #Over-sampling the labeld samples
            ...
            for iteration in range (1, iter_per_epoch): #(zip(train_loader, unlabeled_train_loader)):
                ...
                #Computing the uncertainty/variance map
                ...
                ps_main_loss_un = torch.mean(ps_main_loss*exp_variance_main) #unlabeled samples psuedo-labelnlabeled samples psuedo-label
                ps_1_loss_un = torch.mean(ps_1_loss*exp_variance_aux1) #unlabeled samples psuedo-label
                ps_2_loss_un = torch.mean(ps_2_loss*exp_variance_aux2) #unlabeled samples psuedo-label
                ps_3_loss_un = torch.mean(ps_3_loss*exp_variance_aux3) #unlabeled samples psuedo-label
            ...
            if self.save_best_model_1:
                ...
                torch.save(state_1, Checkpoints_Path + '/UAPS_DAGM_10p.pth')
